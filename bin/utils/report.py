    ...
    for index, row in group_value_df.iterrows():
        ...
        if row['Supervised Fairness'] is False:
            ...
            if row['TypeII Parity'] is False:
                ...
                text3 += ') '
            text2 += text3
        if row['Unsupervised Fairness']:
            if text2 != '':
                text2 += ' neither '
            text2 += 'Unsupervised Fairness '
            text3 = ''
            if row['PPR Parity'] is False:
                text3 += '(no PPR Parity'
...
def audit_report(model_id, parameter, attributes, model_eval, configs, fair_results, fair_measures,
    ...
    if fair_results['Overall Fairness'] is True:
        ...
        if fair_results['Unsupervised Fairness'] is False:
            pdf.cell(0, 5, 'There is unsupervised  at the following attributes: ', 0, 1)

        if fair_results['Supervised Fairness'] is False:
            pdf.set_font('Helvetica', 'B', 11)

            pdf.cell(0, 5, 'There is supervised bias at the following attributes: ', 0, 1)
