...
class Group(object):
    def __init__(self, thresholds):
        ...
        self.fpr = lambda rank_col, label_col, thres, k: lambda x: \
            self.divide(((x[rank_col] <= thres) & (x[label_col] == 0)).sum(),
        ...
        self.fnr = lambda rank_col, label_col, thres, k: lambda x: \
            self.divide(((x[rank_col] > thres) & (x[label_col] == 1)).sum(),
    ...
    # break method in get groups and apply metrics!
    def get_univariate_groups(self, db_conn, models, thresholds, quantizers,
    ...
    def apply_metrics(self, thresholds, df):
